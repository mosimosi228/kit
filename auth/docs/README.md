...
Import: `github.com/mosimosi228/kit/auth`
...
Prefer `New` for an explicit instance. Optional helpers `CreateAuthManager` / `GetAuthManager` keep a process-wide manager.
...
hash, err := a.HashAPIKey(plain)         // HMAC-SHA256 hex (via hmac)
```
...
- `github.com/mosimosi228/kit/hmac`
